...
class ExportServiceTest {
    ...
    void exportsBatchWorkbookWithHeadersCountsAndMissingAttachmentMark() throws Exception {
        ...
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(bytes))) {
            ...
            List<String> headers = List.of("批次名称", "员工姓名", "部门", "金额", "用途分类", "用途说明", "支付时间", "支付凭证数量", "订单截图数量", "发票数量", "提交时间", "管理员备注", "附件目录路径");
            ...
            assertThat(row.getCell(7).getNumericCellValue()).isEqualTo(1);
            assertThat(row.getCell(8).getNumericCellValue()).isEqualTo(0);
            assertThat(row.getCell(9).getNumericCellValue()).isEqualTo(1);
            assertThat(row.getCell(11).getStringCellValue()).isEqualTo("材料齐全");
            assertThat(row.getCell(12).getStringCellValue()).contains("员工一/001-办公用品-128.00").contains("附件缺失");
        }
    }
}
